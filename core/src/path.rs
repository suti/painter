...
impl PathData {
    ...
    pub fn create_rect_reverse(x: f32, y: f32, w: f32, h: f32) -> Option<PathData> {
        Rect::from_xywh(x, y, w, h).and_then(|rect| Some({
            let mut path = PathData::new();
            path.move_to(rect.left(), rect.top());
            path.line_to(rect.left(), rect.bottom());
    ...
    pub fn create_hollow_rect(a: (f32, f32, f32, f32), b: (f32, f32, f32, f32)) -> Option<PathData> {
        Rect::from_xywh(a.0, a.1, a.2, a.3)
            .zip(Rect::from_xywh(b.0, b.1, b.2, b.3))
            .and_then(|(a, b)| {
                if a.left() <= b.left() &&
                    a.top() <= b.right() &&
                    a.right() >= b.right() &&
                    a.bottom() >= b.bottom() {
                    let mut path = PathData::new();
                    // path.move_to(a.left(), a.top());
                    // path.line_to(a.right(), a.top());
                    // path.line_to(a.right(), a.bottom());
                    // path.line_to(a.left(), a.bottom());
                    // path.move_to(b.left(), b.top());
                    // path.line_to(b.left(), b.bottom());
                    // path.line_to(b.right(), b.bottom());
                    // path.line_to(b.right(), b.top());
